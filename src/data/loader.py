...
class PanelSchema:
    ...
    Attributes:
        ...
        is_synthetic: True when this panel came from
            `src.data.synthetic.generate_synthetic_panel` -- either generated
            in this very call, or loaded from a CSV that carries the
...
def _read_synthetic_marker(data_path: str, logger: logging.Logger) -> bool:
    """True when ``data_path`` is a panel written by the synthetic generator.

    Two independent checks, because misclassifying in either direction is
    costly: calling real data synthetic silently sends the official tuned
    parameters to `_dev/`, and calling synthetic data real lets invented
    numbers overwrite them.

    1. A marker file named after this exact panel must exist.
    2. Its ``panel`` field must name this exact file. Guards the case where a
       marker is copied, renamed, or left behind next to a different CSV.
    """
    marker = paths.synthetic_marker_for(data_path)
    ...
    try:
        ...
        # Unreadable marker: assume REAL. The conservative direction is the
        # one that protects the official artifacts from being bypassed
        # silently -- and it is loud, so it gets noticed and fixed.
        logger.warning(
            "Synthetic-provenance marker %s exists but could not be read (%s); "
            "treating this panel as REAL data. Delete the marker if the panel "
            "is generated.", marker, exc,
        )
        return False

    stamped = str(payload.get("panel", ""))
    actual = os.path.basename(data_path)
    if stamped and stamped != actual:
        logger.warning(
            "Synthetic marker %s was written for panel %r but sits beside %r; "
            "ignoring it and treating this panel as REAL data.",
            marker, stamped, actual,
        )
        return False

    logger.info(
        "Panel carries a synthetic-provenance marker (%s); this run is NOT "
        "official and will not persist tuned parameters.", marker,
    )
    return bool(payload.get("synthetic", True))
...
def load_or_generate_panel(
    ...
    with log_phase("load_or_generate_panel", logger):
        ...
        if not os.path.exists(data_path):
            ...
            # A panel generated by an earlier run is still synthetic. The
            # marker is the only thing that survives to say so -- on disk the
            # CSV is indistinguishable from real data.
            is_synthetic = _read_synthetic_marker(data_path, logger)

        df = pd.read_csv(data_path)
        logger.info("Loaded panel: %d rows x %d columns from '%s'", len(df), df.shape[1], data_path)

        schema = _infer_schema(
            df, logger, ground_truth_path=ground_truth_path,
            is_synthetic=is_synthetic,
        )
